      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Edit Data Pejabat</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
    ...
    <div class="container-fluid">
      <div class="card card-primary">
        <div class="card-header">
          ...
          <button class="btn btn-warning btn-sm float-right" onClick="location.href='dashboard.php?p=lihat-pejabat'"><i class="fas fa-eye nav-icon"></i></button>
        </div>
        <div class="card-body">
          <form action="update-pejabat.php" method="post">
          <!-- Row 1-->
          <?php 
            error_reporting(E_ALL & ~E_NOTICE);
            include "../koneksi.php"; 
            $id_pegawai=$_GET['id_pejabat']; 
            $query_mysql=mysqli_query($konek, "SELECT * FROM pejabat WHERE id_pejabat='$id_pegawai'")or die(mysqli_error()); 
          ...
          <div class="row">
            <div class="col-4">
              <div class="form-group">
                ...
                <input type="text" class="form-control" name="id_pejabat" value="<?php echo $data['id_pejabat']?>" readonly>
              </div>
            </div>
            <div class="col-4">
              <div class="form-group">
                <label>Nama Pejabat</label>
                <input name="nama_pejabat" class="form-control" value="<?php echo $data['nama_pejabat']?>" required autofocus>
              </div>
            </div>
            <div class="col-4">
              <div class="form-group">
                <label>NIP</label>
                <input name="nip_pejabat" type="number" class="form-control" value="<?php echo $data['nip_pejabat']?>" required autofocus>
            </div>
          </div>
        </div>
          <!-- ./ Row 1-->
                ...
                <label>Jabatan</label>
                    ...
                    <option selected disabled></option>
                      <?php 
                          ...
                          while($bat = mysqli_fetch_assoc($djabat))
                          {
                          ?>
                    <option><?php echo $bat['nama_jabatan']; ?></option>
                        <?php } ?>
                  </select>
              </div>
            </div>
                ...
                <select name="golongan_pegawai" class="form-control select2" required>
                    <option selected disabled></option>
                      <?php 
                          ...
                          while($wai = mysqli_fetch_assoc($djabat))
                          {
                          ?>
                    <option><?php echo $wai['nama_golongan']; ?></option>
                        <?php } ?>
                  </select>
            </div>
          </div>
